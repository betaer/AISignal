...
AI Signal Guard 先让你选择目标身份，再运行检测，并用画像专属权重生成 Identity Match Score、证据覆盖率、自然语言总结以及“为什么像 / 为什么不像”。同一组环境信号在 AI 用户、自媒体创作者和跨境商家画像下会得到不同解释。
...
| 画像 | 主要分析信号 |
|---|---|
| 🤖 AI 用户 | ChatGPT、OpenAI、Claude、Gemini、Perplexity 等常用 AI 服务的访问状态、网络路径与环境一致性；Cursor、GitHub、npm、PyPI 仅作补充诊断，不作为评分前提 |
