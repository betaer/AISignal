...
- 11 个确定性浏览器场景、40 项断言通过。
- 已覆盖六个 SVG 节点、有效 symbol 引用、悬停气泡跨异步评分更新连续可见、悬停/焦点/固定气泡三级优先级、Escape 全关闭并保持触发器焦点、异步重渲染焦点保持、WebRTC 红色状态、390px 与 300px 无横向溢出。
...
3. 修复后自动化证据：40/40 通过；最终同画布视觉复核仍因内置浏览器不可用而阻塞。
